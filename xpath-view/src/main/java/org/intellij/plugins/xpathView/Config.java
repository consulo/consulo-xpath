...
public class Config {
  ...
  public boolean equals(Object o) {
    ...
    if (HIGHLIGHT_RESULTS != config.HIGHLIGHT_RESULTS) {
      return false;
    }
    if (OPEN_NEW_TAB != config.OPEN_NEW_TAB) {
      return false;
    }
    if (SHOW_IN_MAIN_MENU != config.SHOW_IN_MAIN_MENU) {
      return false;
    }
    if (SHOW_IN_TOOLBAR != config.SHOW_IN_TOOLBAR) {
      return false;
    }
    if (SHOW_USAGE_VIEW != config.SHOW_USAGE_VIEW) {
      return false;
    }
    if (bAddErrorStripe != config.bAddErrorStripe) {
      return false;
    }
    if (bHighlightStartTagOnly != config.bHighlightStartTagOnly) {
      return false;
    }
    if (bUseContextAtCursor != config.bUseContextAtCursor) {
      return false;
    }
