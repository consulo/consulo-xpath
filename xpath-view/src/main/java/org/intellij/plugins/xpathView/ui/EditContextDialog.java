    ...
    final JPanel n = ToolbarDecorator.createDecorator(myNamespaceTable)
                                     .setAddAction(myContextProvider.getContextElement() != null ? null : new AnActionButtonRunnable() {
                                       ...
                                       public void run(AnActionButton button) {
                                         ...
                                         final AddNamespaceDialog dlg =
                                           new AddNamespaceDialog(project, myUnresolvedPrefixes, allURIs, AddNamespaceDialog.Mode.EDITABLE);
                                         dlg.show();
                                         if (dlg.isOK()) {
    ...
    for (int i = 0; i < expressions.size(); i++) {
      ...
      if (error != null) {
        Messages.showErrorDialog(expression.getFile().getProject(),
                                 "Error in XPath Expression for Variable '" + name + "': " + error,
                                 "XPath Error");
